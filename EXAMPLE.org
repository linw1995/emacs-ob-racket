...
The body is in =racket=, and we are able to pass in related expressions for it.
#+BEGIN_SRC racket :var x='pi :var y='(/ x pi) :command '(spaced "strings" :in-file)
(+ x y)
#+END_SRC
...
The body is in another lang, and we are able to pass in values for it.
#+HEADER: :command '(spaced "strings" :in-file)
#+HEADER: :define-var '(define-var-name " = " define-var-val "\n")
#+BEGIN_SRC racket :var x=1 :var y=2 :lang anohter :results output
z = x + y
#+END_SRC
